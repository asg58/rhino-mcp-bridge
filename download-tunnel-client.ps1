...
$asset = $assets | Where-Object {
    $_.name -match '(?i)^tunnel-client-v.*-windows-amd64\.zip$'
} | Select-Object -First 1

if (-not $asset) {
    Write-Host "Exact Windows amd64 tunnel-client ZIP niet automatisch gevonden. Beschikbare Windows assets:" -ForegroundColor Yellow
    $assets | Where-Object { $_.name -match '(?i)windows' } | ForEach-Object { Write-Host $_.name }
    throw "Download de tunnel-client handmatig en zet tunnel-client.exe in .tools"
}
